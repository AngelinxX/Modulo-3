...
public class ClientesBDD {
	...
	public void actualizar(Cliente cliente) throws KrakeDevException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement(
					"UPDATE public.clientes" + "	SET nombre=?, numerohijos=?" + "	WHERE cedula = ?");
	...
	public ArrayList<Cliente> buscarPorNumHijos(int numeroHijosBuscados) throws KrakeDevException {
		ArrayList<Cliente> clientes = new ArrayList<Cliente>();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			ps.setInt(1, numeroHijosBuscados);
			rs = ps.executeQuery();
